...
func TestGraylogSourcesFromVpnConfig(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO vpn_config VALUES
		('FGT100F_VERITAS-HQ', 'veritas-hq', 'veritas-hq.example.com', 'FGT100F_VERITAS-HQ-N1, FGT100F_VERITAS-HQ-N2'),
		('FGT40F_Agora-HL', 'agora', 'agora.example.com', '')`); err != nil {
		t.Fatal(err)
	}
	_ = db.Close()
	...
	got := e.graylogSources("veritas-hq.example.com")
	if len(got) != 2 || got[0] != "FGT100F_VERITAS-HQ-N1" || got[1] != "FGT100F_VERITAS-HQ-N2" {
		t.Fatalf("cluster sources wrong: %v", got)
	}
	// Standalone (no cluster_hostnames): the firewallname, matched case-insensitively.
	if got := e.graylogSources("agora.example.com"); len(got) != 1 || got[0] != "FGT40F_Agora-HL" {
