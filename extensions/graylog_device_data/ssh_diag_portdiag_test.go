...
func TestValidDiagName(t *testing.T) {
	valid := []string{"port1", "port48", "EX-CORE01", "EX_ACCESS.03", "S424EPTF20000976"}
	for _, s := range valid {
		if !ValidDiagName(s) {
...
func TestCleanDiagOutput(t *testing.T) {
	raw := "diagnose switch-controller switch-info port-stats EX-CORE01 port1\n" +
		"\n" +
		"Vdom: root\n" +
		"Port(port1) is HW Admin up, SW Admin up, line protocol is up\n" +
		"full-duplex, 1000 Mb/s\n" +
		"\n" +
		"FGT90GEWORX-N2(Primary) $ "
