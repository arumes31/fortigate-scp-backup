...
test('login form stages TOTP and supports keyboard use without disclosing the account type', async ({ page }) => {
  await page.goto('/login', { waitUntil: 'networkidle' });

  const username = page.getByLabel('Username');
  const password = page.getByLabel('Password', { exact: true });
  const totp = page.getByLabel('TOTP Code');
  const radiusBanner = page.locator('#radius-banner');
  ...
  await expect(radiusBanner).toBeHidden();
...
test('login waiting state is announced without changing submitted values', async ({ page }) => {
  ...
  const status = page.locator('#login-wait');
