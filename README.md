...
| `FGT_CONFTAIL_RETENTION_DAYS` | `365` | Days to retain sealed, terminal history after Hookwise acceptance or a manual queue clear. `0` disables pruning; active or still-queued work is never pruned. |
...
* **Operational visibility** — `/fgt-conftail` separates Graylog collector, session, and Hookwise health; refreshes only when collector state changes; offers firewall autocomplete plus source/device/serial/action and exact transaction/log-ID filters; and lets operators switch displayed timestamps between UTC and the browser time zone. Operators can clear every unaccepted Hookwise delivery after confirmation without deleting configuration-change history. Source coverage is available only with ADM VPN Config, includes only Graylog-enabled firewalls, and stays collapsed by default. Accepted ticket lifecycle actions remain in Hookwise/ConnectWise.
...
* **Object & group reuse, gap list** — existing address/service objects *and* address/service groups from the latest config backup are referenced on exact match (including exact port-range objects); everything missing is listed explicitly ("objects to create"); new address objects/groups are generated under a configurable name prefix, while new services carry plain protocol names (`TCP443`, `UDP53`, `TCP8080-8082`) or the service name seen in the logs (`RDP`). Adjacent single ports consolidate into range objects.
